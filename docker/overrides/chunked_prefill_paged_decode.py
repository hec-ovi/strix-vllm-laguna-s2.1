...
def kernel_paged_attention_2d(
    ...
    # Sliding-window skip: every token below (seq_len - SLIDING_WINDOW) is
    # score-masked out below anyway, so their blocks never influence the
    # output. Starting the loop at the window edge means their K/V is never
    # read from cache; the existing mask still trims the partial first block.
    # Decode is bandwidth-bound, so for a 512-window layer at 16K context this
    # removes ~97% of the KV reads.
    if SLIDING_WINDOW > 0:
